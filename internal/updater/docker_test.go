...
func (f *fakeClient) addContainer(id, name, image string) {
...
func (f *fakeClient) addImage(ref, id string) {
...
func (f *fakeClient) FindContainerByID(_ context.Context, id string) (*dockercontainer.Summary, error) {
...
func (f *fakeClient) ContainerInspect(_ context.Context, id string, _ dockerclient.ContainerInspectOptions) (dockerclient.ContainerInspectResult, error) {
	if f.inspectErr != nil {
...
func (f *fakeClient) ImageInspect(_ context.Context, ref string, _ ...dockerclient.ImageInspectOption) (dockerclient.ImageInspectResult, error) {
	res, ok := f.images[ref]
	if !ok {
...
func (f *fakeClient) ContainerCreate(_ context.Context, opts dockerclient.ContainerCreateOptions) (dockerclient.ContainerCreateResult, error) {
	if f.createErr != nil {
...
func (f *fakeClient) ContainerStart(_ context.Context, id string, _ dockerclient.ContainerStartOptions) (dockerclient.ContainerStartResult, error) {
	f.started = append(f.started, id)
	return dockerclient.ContainerStartResult{}, f.startErr
}

func (f *fakeClient) ContainerStop(_ context.Context, id string, _ dockerclient.ContainerStopOptions) (dockerclient.ContainerStopResult, error) {
	f.stopped = append(f.stopped, id)
	return dockerclient.ContainerStopResult{}, f.stopErr
}

func (f *fakeClient) ContainerRemove(_ context.Context, id string, _ dockerclient.ContainerRemoveOptions) (dockerclient.ContainerRemoveResult, error) {
	f.removed = append(f.removed, id)
	return dockerclient.ContainerRemoveResult{}, f.removeErr
}

func (f *fakeClient) ContainerRename(_ context.Context, _ string, opts dockerclient.ContainerRenameOptions) (dockerclient.ContainerRenameResult, error) {
...
func TestUpdateDockerAlreadyUpToDate(t *testing.T) {
	f := newFakeClient()
	f.addImage("nginx:1.25", "sha256:same")
	u := newTestUpdater(f)

	c := dockercontainer.Summary{ID: "c1", Names: []string{"/nginx"}, Image: "nginx:1.25", ImageID: "sha256:same"}
...
func TestUpdateDockerRecreates(t *testing.T) {
	f := newFakeClient()
	f.addImage("nginx:1.25", "sha256:new")
	f.addContainer("c1", "nginx", "nginx:1.25")
	u := newTestUpdater(f)

	c := dockercontainer.Summary{ID: "c1", Names: []string{"/nginx"}, Image: "nginx:1.25", ImageID: "sha256:old"}
	u.updateDocker(context.Background(), c)
...
func TestUpdateDockerRecoversDanglingImage(t *testing.T) {
	...
	c := dockercontainer.Summary{ID: "c1", Names: []string{"/nginx"}, Image: "sha256:deadbeef", ImageID: "sha256:old"}
	u.updateDocker(context.Background(), c)
...
func TestUpdateDockerSelfSkip(t *testing.T) {
	...
	c := dockercontainer.Summary{ID: "c1", Names: []string{"/orbitd"}, Image: "nginx:1.25", ImageID: "sha256:old"}
	u.updateDocker(context.Background(), c)
